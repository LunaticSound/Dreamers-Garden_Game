...
const float normalMidFix = 0.00392; // (128.0/255.0)*2.0-1.0
...
vec2 ApplyShockwavesUV(vec2 uv, out vec2 offsetOut) {
	vec3 normal = clamp(texture2D(u_shockwavesTexture, mix(u_shockwavesUVs.xy, u_shockwavesUVs.zw, uv)).rgb, 0.0, 1.0) * 2.0-1.0;
	vec2 offset = ((normal.xy-normalMidFix) * u_shockwavesAmount);
	offsetOut = offset;
	return uv + offset;
}
...
vec2 ApplyDisplaceMapsUV(vec2 uv) {
	...
	vec3 normal = clamp(texture2D(u_displacemapTexture, mix(u_displacemapUVs.xy, u_displacemapUVs.zw, uv2)).rgb, 0.0, 1.0) * 2.0-1.0;
	return uv + ((normal.xy-normalMidFix) * u_displacemapAmount);
}
