...
function __PPFX_DebugInspectorGenerator() constructor {
	...
	static __PPFX_Renderer = function() {
		...
		gui_ef_set_param_simple = function(value, effect_enum, param) {
			if (__inspected_class.__effectsOrderedAmount == 0 || !__inspected_class.GetEffectExists(effect_enum)) exit;
		...
		gui_ef_set_enabled = function(checked, effect_enum) {
			//if (!__inspected_class.GetEffectExists(effect_enum)) exit;
			var _check = !checked;
		...
		gui_ef_get_param_simple = function(effect_enum, param) {
			if (__inspected_class.__effectsOrderedAmount == 0 || !__inspected_class.GetEffectExists(effect_enum)) return 0;
			return __inspected_class.GetEffectParameter(effect_enum, param);
		}
		gui_ef_get_enabled = function(effect_enum) {
			if (__inspected_class.__effectsOrderedAmount == 0 || !__inspected_class.GetEffectExists(effect_enum)) return false;
		...
		gui_color_curves_get_graph = function() {
			if (__inspected_class.__effectsOrderedAmount > 0) {
				var _effect_struct = __inspected_class.GetEffect(FF_COLOR_CURVES);
		...
		inspector_struct
		.AddElement(new __ppf_ui_category("Lighting & Lens", false, 145));
		...
		inspector_struct
		.AddElement(new __ppf_ui_menu("DEPTH OF FIELD", false, 20))
			...
			.AddElement(new __ppf_ui_slider("Intensity", 0, false, gui_ef_get_param_simple(FF_DEPTH_OF_FIELD, PP_DOF_INTENSITY), 0, 1, true, function(output) {gui_ef_set_param_simple(output, FF_DEPTH_OF_FIELD, PP_DOF_INTENSITY);}))
				.AddElement(new __ppf_ui_folder("Shape", true, 2))
				...
				.AddElement(new __ppf_ui_folder("Z Depth", false, 7))
					.AddElement(new __ppf_ui_text_ext("Info", 0, function(element) {
						element.width = element.parent.__panel_width-element.xx-element.parent.__element_xpadding;
						var _text = "Note: the depth buffer must be enabled to work with the scene.";
						return _text;
					}))
					.AddElement(new __ppf_ui_checkbox("Enabled", 0, gui_ef_get_param_simple(FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_ENABLE), function(checked) {return gui_ef_set_param_enable(checked, FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_ENABLE);}))
					.AddElement(new __ppf_ui_button("Texture", function() {
						var _file = get_open_filename("PNG Image|*.png", "*.png");
						if (_file != "") {
					...
					.AddElement(new __ppf_ui_checkbox("Linearize (for 3D usage)", 0, false, function(checked) {return gui_ef_set_param_enable(checked, FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_LINEARIZE);}))
					.AddElement(new __ppf_ui_slider("Near Plane", 0, false, gui_ef_get_param_simple(FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_NEAR), 1, 2, true, function(output) {gui_ef_set_param_simple(output, FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_NEAR);}))
					.AddElement(new __ppf_ui_slider("Far Plane", 0, false, gui_ef_get_param_simple(FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_FAR), 1, 32000, true, function(output) {gui_ef_set_param_simple(output, FF_DEPTH_OF_FIELD, PP_DOF_ZDEPTH_FAR);}))
				...
				.AddElement(new __ppf_ui_slider("Focus Smoothness", 0, false, gui_ef_get_param_simple(FF_DEPTH_OF_FIELD, PP_DOF_FOCUS_SMOOTHNESS), 0, 1, true, function(output) {gui_ef_set_param_simple(output, FF_DEPTH_OF_FIELD, PP_DOF_FOCUS_SMOOTHNESS);}))
		...
		// effect: Glitch
		inspector_struct
		.AddElement(new __ppf_ui_menu("GLITCH (Experimental)", false, 21))
