...
function PPFX_Renderer() constructor {
	...
	static __stackGetSurfacePrevious = function() {
		return __stackSurfaces[max(__stackIndex-1, 0)];
	}
	...
	///
	/// IMPORTANT: When restarting rooms, or switching rooms, the layer script will be removed from the layers by GameMaker, so you need to call .LayerApply() again, or .LayerSetRange(), like in the Room Start event, otherwise the renderer will not render on the layer, you created it on the Create event only.
	///
	...
	static LayerApply = function(_topLayer, _bottomLayer=_topLayer) {
			...
			__layerBeginFunc = function() {
				if (event_type == ev_draw && event_number == 0) {
					...
					if (surface_exists(__layerSurface)) {
						if (surface_get_width(__layerSurface) != _sourceWidth || surface_get_height(__layerSurface) != _sourceHeight) {
					...
					// renderize layer contents inside the surface
						...
						matrix_set(matrix_view, camera_get_view_mat(__layerWorldCamera));
						matrix_set(matrix_projection, camera_get_proj_mat(__layerWorldCamera));
						gpu_push_state();
		...
		/// @ignore
		if (__layerEndFunc == undefined) {
			__layerEndFunc = function() {
				if (event_type == ev_draw && event_number == 0 && surface_exists(__layerSurface)) {
						gpu_pop_state();
						surface_reset_target();
					// draw it at the camera position (with support for cameras created via matrix functions)
					var _oldDepth = gpu_get_depth();
					gpu_set_depth(layer_get_depth(__layerTopLayer));
					var _viewMat = camera_get_view_mat(__layerWorldCamera);
					var _projMat = camera_get_proj_mat(__layerWorldCamera);
					var _xx = -_viewMat[12] - round(abs(2/_projMat[0]))/2;
					var _yy = -_viewMat[13] - round(abs(2/_projMat[5]))/2;
					var _ww = round(abs(2/_projMat[0]));
					var _hh = round(abs(2/_projMat[5]));
					Draw(__layerSurface, _xx, _yy, _ww, _hh);
	...
	static LayerSetRange = function(_topLayer, _bottomLayer=_topLayer) {
		if (__layerBeginFunc == undefined || __layerEndFunc == undefined || __layerTopLayer == undefined || __layerBottomLayer == undefined) exit;
		if (layer_get_script_begin(__layerBottomLayer) == __layerBeginFunc) layer_script_begin(__layerBottomLayer, -1);
		if (layer_get_script_end(__layerTopLayer) == __layerEndFunc) layer_script_begin(__layerTopLayer, -1);
		__layerTopLayer = _topLayer;
		__layerBottomLayer = _bottomLayer;
		layer_script_begin(__layerBottomLayer, __layerBeginFunc);
		layer_script_end(__layerTopLayer, __layerEndFunc);
	...
	/// @desc Removes the renderer from the previously defined layer range and deletes the range surface from the memory.
	/// @method LayerRemove()
	static LayerRemove = function() {
		if (__layerTopLayer == undefined || __layerBottomLayer == undefined) exit;
		if (layer_get_script_begin(__layerBottomLayer) == __layerBeginFunc) layer_script_begin(__layerBottomLayer, -1);
		if (layer_get_script_end(__layerTopLayer) == __layerEndFunc) layer_script_begin(__layerTopLayer, -1);
		if (surface_exists(__layerSurface)) surface_free(__layerSurface);
	}
	...
	static ProfileLoad = function(_profile, _merge=false, _filter=undefined) {
		...
		if (__currentProfile != _profile) {
			var _profileEffectsArray = _profile.__effectsOrdered,
	...
	/// @desc Destroy Post-processing renderer. You should only call this when you are sure you will not use this same renderer instance again.
	/// @method Destroy()
	/// @returns {undefined}
	...
	/// @param {Bool} enable Will be either true (enabled, the default value) or false (disabled). The drawing will toggle if nothing or -1 is entered.
	/// @param {Bool} alphaBlending If true, you will be able to use the renderer with a transparent surface. This is false by default, for performance reasons.
	/// @returns {undefined}
	static SetDrawEnable = function(_enable=-1, _alphaBlending=false) {
		if (_enable == -1) {
			__isDrawEnabled = !__isDrawEnabled;
		} else {
			__isDrawEnabled = _enable;
		}
		__enableBlending = _alphaBlending;
		return self;
	}
	...
	static SetRenderEnable = function(_enable=-1, _clearMemory=false) {
		...
		if (!__isRenderEnabled) {
			if (_clearMemory) __stacksClean();
			LayerRemove();
		} else {
			LayerSetRange(__layerTopLayer, __layerBottomLayer);
		}
		return self;
	}
	...
	static SetEffectParameter = function(_effect, _param, _value) {
		...
		if (_effectStruct == undefined) {
			__ppf_trace($"'{_effect}' effect does not exist in the {instanceof(self)} instance (not loaded from a profile).", 1);
			exit;
		}
	...
	/// @desc Get specific effect struct. With this, you can also access the effect variables directly.
	/// @param {String} effect The effect name for searching.
	/// @method GetEffect(effect)
	static GetEffect = function(_effect) {
		return __effects[$ _effect];
	}
	
	/// @desc Check if some effect exists in this renderer.
	/// @param {String} effect The effect name to check if exists.
	/// @method GetEffectExists(effect)
	static GetEffectExists = function(_effect) {
	...
	/// @desc This function returns the reference to the loaded PPFX_Profile(). Note: this is only the reference to the PPFX_Profile() struct you loaded! Any changes to the returned value will reflect the reference and have no bearing on the Renderer.
	/// @method GetProfile()
	static GetProfile = function() {
		return __currentProfile;
	}
	...
	/// NOTE: If you're just rendering, position and size are optional.
	...
	/// @returns {undefined}
	static Draw = function(_surface, _x=0, _y=0, _w=0, _h=0) {
		...
		__cpuFrameTime = 0;
		if (__isRenderEnabled) {
			// read-only drawing location, used by areas
			__drawX = _x; __drawY = _y; __drawW = _w; __drawH = _h;
		...
		if (__isDrawEnabled) {
			...
			draw_surface_stretched(__renderSurfaceOutput, _x, _y, _w, _h);
		}
		gpu_pop_state();
		return __renderSurfaceOutput;
	}
	...
	static DrawInFullscreen = function(_surface=application_surface) {
		...
		return Draw(_surface, _xx, _yy, _width, _height);
	}
