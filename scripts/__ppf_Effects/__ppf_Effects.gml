...
function __ST_ColorGrading() : __PPF_STSuperClass() constructor {
	...
	static End = function(_renderer, _surfaceWidth, _surfaceHeight, _time) {
			...
			if (sprite_exists(_renderer.__miscLinearLUTSprite)) {
				...
				var _isUsingToneMapping = (_renderer.GetEffectExists(FF_TONE_MAPPING) && _renderer.IsEffectEnabled(FF_TONE_MAPPING));
...
function __ST_Final() : __PPF_STSuperClass() constructor {
	...
	static Start = function(_renderer, _surfaceWidth, _surfaceHeight, _time) {
		...
		_renderer.__stackSetSurface(_surfaceWidth, _surfaceHeight, stackName);
			...
			var _lensEffect = _renderer.GetEffect(FF_LENS_DISTORTION);
...
/// @desc The Bloom effect makes emissive (bright) areas in your image glow, making a realistic simulation of light. The effect also has dirt-lens to simulate a dirty camera (optional).
...
/// @param {Real} threshold Bloom is only applied to pixels in the scene that have emission/exposure GREATER than this value. The minimum value is 0, where nothing is filtered. There is no maximum value.
...
/// Note that the application_surface does not support HDR out of the box, so you will need to use the "Crystal Lighting Engine" or the "Rezol" libraries.
/// @param {Real} knee Controls the softness of the transition between the bloomed and non-bloomed areas of the image. 0 - 1.
/// @param {Real} intensity Set the strength of the Bloom filter. 0 to 5 recommended. There is no maximum amount.
...
/// @param {Bool} dirtEnable Defines whether to use dirt textures. true or false (default).
...
/// @param {Real} dirtTiled Defines whether the dirt lens will repeat seamlessly. true or false.
/// @param {Bool} dirtCanDistort If active, the dirt texture will distort according to the lens distortion effect. true or false.
...
function FX_Bloom(_enabled, _iterations=8, _threshold=0.4, _knee=0, _intensity=3, _color=c_white, _whiteAmount=0, _dirtEnable=false, _dirtTexture=undefined, _dirtIntensity=2.5, _dirtScale=1, _dirtTiled=true, _dirtCanDistort=false, _resolution=1, _debug1=false, _debug2=false) : __PPF_FXSuperClass() constructor {
	...
	dirtTexture = _dirtTexture ?? sprite_get_texture(__ppf_sprDirtLens, 0);
...
function FX_SunShafts(_enabled, _position=[0.5, 0.5], _threshold=0.5, _intensity=3, _dimmer=0.5, _scattering=1, _raysNoiseEnable=false, _raysIntensity=1, _raysTiling=1, _raysSpeed=0.1, _raysOcclusionRadius=0.06, _raysOcclusionSmoothness=0.1, _raysNoiseTex=undefined, _resolution=1, _debug=false, _sourceOffset=0) : __PPF_FXSuperClass() constructor {
	...
	raysNoiseTex = _raysNoiseTex ?? sprite_get_texture(__ppf_sprNoisePerlin, 0);
	resolution = _resolution;
	debug = _debug;
...
/// @param {Bool} zDepthEnable Defines if the DOF will use a depth map.
...
/// @param {Real} zDepthLinearize If true, linearize the depth texture based on the zNear and zFar planes.
/// @param {Real} zDepthNear The near plane of the camera. Example: 1.
/// @param {Real} zDepthFar The far plane of the camera. Example: 16000.
...
/// @param {Real} focusSmoothness The smoothness between the focus point and the focus range.
...
function FX_DepthOfField(_enabled, _radius=10, _intensity=1, _bladesAperture=6, _bladesAngle=0, _useZdepth=false, _zDepthTexture=undefined, _zDepthLinearize=false, _zDepthZNear=1, _zDepthZFar=16000, _focusDistance=0.2, _focusRange=0.2, _focusSmoothness=0, _focusNear=1, _focusFar=1, _resolution=0.5, _debug=false) : __PPF_FXSuperClass() constructor {
	...
	zDepthEnable = _useZdepth;
	zDepthTexture = _zDepthTexture;
	zDepthLinearize = _zDepthLinearize;
	zDepthNear = _zDepthZNear;
	zDepthFar = _zDepthZFar;
	focusDistance = _focusDistance;
	focusSmoothness = _focusSmoothness;
	...
	static u_cocFocusWidth = shader_get_uniform(__ppf_shRenderDOFBokeh, "u_focusWidth");
	...
	static u_cocZDepthLinearize = shader_get_uniform(__ppf_shRenderDOFBokeh, "u_zDepthLinearize");
	...
	static u_zDepthNearFar = shader_get_uniform(__ppf_shRenderDOFBokeh, "u_zDepthNearFar");
	...
	dofPostFilterSurf = -1;
	oldDofResolution = 0;
	whitePixelTexture = sprite_get_texture(__ppf_sprPixel, 0);
	canRender = true;
	...
	static Draw = function(_renderer, _surfaceWidth, _surfaceHeight, _time) {
		...
		surface_set_target(dofBokehSurf); // destination
			...
			shader_set_uniform_f(u_cocFocusWidth, 1-max(focusSmoothness, PPFX_CFG_EPSILON));
			shader_set_uniform_f(u_cocFocusNear, focusNear);
			shader_set_uniform_f(u_cocFocusFar, focusFar);
			shader_set_uniform_f(u_cocZDepthLinearize,  zDepthLinearize);
			var _lensEffect = _renderer.GetEffect(FF_LENS_DISTORTION);
			if (_lensEffect != undefined) {
				shader_set_uniform_f(u_d_lensDistortionEnable, _lensEffect.enabled);
				shader_set_uniform_f(u_d_lensDistortionAmount, _lensEffect.amount);
			...
			shader_set_uniform_f(u_bokehBladesAperture, pi/max(3, bladesAperture));
			shader_set_uniform_f(u_bokehBladesAngle, degtorad(bladesAngle));
			shader_set_uniform_f(u_debug, debug);
			
			shader_set_uniform_f(u_zDepthNearFar, zDepthNear, zDepthFar);
			gpu_set_tex_filter_ext(u_zDepthTexture, false);
			if (zDepthEnable) {
				texture_set_stage(u_zDepthTexture, zDepthTexture ?? surface_get_texture_depth(_renderer.__stackSurfaces[0]));
			} else {
	...
	/// @desc Calculates the focus distance based on the position (in world-space) to be used with the Depth of Field effect.
	/// @param {Real} x The x position in world-space.
	/// @param {Real} y The y position in world-space.
	/// @param {Real} z The z position in world-space.
	/// @param {ID.GMCamera} camera The camera create with camera_create() or from a viewport: view_get_camera().
	/// @method CalculateFocusDistance(x, y, z, camera)
	static CalculateFocusDistance = function(_x, _y, _z, _camera) {
		var _viewMat = camera_get_view_mat(_camera);
		var _projMat = camera_get_proj_mat(_camera);
		var _viewPos = matrix_transform_vertex(_viewMat, _x, _y, _z); // transform to view-space
		var _viewDepth = _viewPos[2]; // view-space z
		zDepthNear = -2 * _projMat[14] / (_projMat[10]+1);
		zDepthFar = -_projMat[14] / (_projMat[10]-1);
		focusDistance = clamp(_viewDepth / (zDepthFar/2), 0, 1);
		return self;
	}
	...
	static ExportData = function() {
		return {
			name : instanceof(self),
			params : [enabled, radius, intensity, bladesAperture, bladesAngle, zDepthEnable, zDepthTexture, zDepthLinearize, zDepthNear, zDepthFar, focusDistance, focusRange, focusSmoothness, focusNear, focusFar, resolution, debug],
		};
	}
}
...
function FX_ChromaticAberration(_enabled, _intensity=1, _angle=35, _inner=1, _centerRadius=0, _blurEnable=false, _prismaLUTtexture=undefined) : __PPF_FXSuperClass() constructor {
	...
	prismaLutTexture = _prismaLUTtexture ?? sprite_get_texture(__ppf_sprPrismLutRB, 0);
...
function FX_ASCII(_enabled, _intensity=1, _saturation=1, _mix=0, _pixelMix=1, _scale=1, _color=c_white, _texture=undefined, _framesAmount=8, _gridCellWidth=16, _gridCellHeight=16) : __PPF_FXSuperClass() constructor {
	...
	scale = _scale;
	color = make_color_ppfx(_color);
	texture = _texture ?? sprite_get_texture(__ppf_sprASCII, 0);
...
/// @desc Super cool Glitch effect. THIS IS EXPERIMENTAL!!! It may change in any update. This is just a preview. DO NOT USE IT IN PRODUCTION.
function FX_Glitch(_enabled, _intensity=1) : __PPF_FXSuperClass() constructor {
	...
	static Draw = function(_renderer, _surfaceWidth, _surfaceHeight, _time) {
		...
		_renderer.__stackSetSurface(_surfaceWidth, _surfaceHeight, effectName);
			...
			//var _lens_effect = _renderer.GetEffect(FF_LENS_DISTORTION);
...
function FX_PaletteSwap(_enabled, _row=1, _flip=false, _texture=undefined, _paletteHeight=1, _threshold=0, _smoothness=0, _limitColors=true) : __PPF_FXSuperClass() constructor {
	...
	row = _row;
	flip = _flip;
	texture = _texture ?? sprite_get_texture(__ppf_sprPalDefault, 0);
...
/// @param {Bool} enabled Defines whether the effect starts active or not.
/// @param {Real} strength Anti-aliasing strength. 2 - 8 works best.
...
/// @param {Bool} enabled Defines whether the effect starts active or not.
/// @param {Real} intensity Sets the intensity level of the effect. 0 - 1. 0.85 recommended. 
...
function FX_RadialBlur(_enabled, _radius=0.5, _inner=1.5, _center=[0.5,0.5]) : __PPF_FXSuperClass() constructor {
	...
	static Draw = function(_renderer, _surfaceWidth, _surfaceHeight, _time) {
		...
		_renderer.__stackSetSurface(_surfaceWidth, _surfaceHeight, effectName);
			...
			shader_set_uniform_f_array(u_center, center);
			shader_set_uniform_f(u_inner, max(inner, PPFX_CFG_EPSILON));
			gpu_set_tex_repeat_ext(u_noiseTexture, true);
			shader_set_uniform_f(u_noiseUVs, noiseUVs[0], noiseUVs[1], noiseUVs[2], noiseUVs[3]);
			texture_set_stage(u_noiseTexture, noiseTexture);
			shader_set_uniform_f(u_noiseSize, noiseWidth, noiseHeight);
...
function FX_TextureOverlay(_enabled, _intensity=1, _scale=1, _texture=undefined, _blendmode=0, _canDistort=false, _tiled=false) : __PPF_FXSuperClass() constructor {
	...
	intensity = _intensity;
	scale = _scale;
	texture = _texture ?? sprite_get_texture(__ppf_sprNoisePoint, 0);
...
#region FOG (TO-DO)
/*
function FX_Fog(_enabled, _intensity=1, _depthTexture=undefined) : __PPF_FXSuperClass() constructor {
	effectName = "fog_deferred";
	stackOrder = PPFX_STACK.FOG_DEFERRED;
...
function FX_Shockwaves(_enabled, _amount=0.1, _aberration=0.1, _prismaLutTexture=undefined, _texture=undefined) : __PPF_FXSuperClass() constructor {
	...
	prismaLutTexture = _prismaLutTexture ?? sprite_get_texture(__ppf_sprPrismLutCP, 0);
	texture = _texture ?? sprite_get_texture(__ppf_sprNormal, 0);
...
function FX_LUT(_enabled, _intensity=1, _type=1, _horizontalSquares=8, _texture=undefined) : __PPF_FXSuperClass() constructor {
	...
	texture = _texture ?? sprite_get_texture(__ppf_sprNeutralLUTGrid, 0);
...
/// @param {Real} fadeAngle Partial fade angle.
 function FX_Mist(_enabled, _intensity=0.5, _scale=0.5, _tiling=1, _speedd=0.2, _angle=0, _contrast=0.8, _power=1, _remap=0.8, _color=c_white, _mix=0, _mixThreshold=0, _noiseTex=undefined, _offset=[0.0,0.0], _fadeAmount=0, _fadeAngle=270) : __PPF_FXSuperClass() constructor {
	...
	noiseTex = _noiseTex ?? sprite_get_texture(__ppf_sprNoisePerlin, 0);
	offset = _offset;
...
function FX_SpeedLines(_enabled, _scale=0.1, _tiling=5, _speed=2, _rotSpeed=1, _contrast=0.5, _power=1, _remap=0.8, _color=c_white, _maskPower=5, _maskScale=1.2, _maskSmoothness=1, _noiseTexture=undefined) : __PPF_FXSuperClass() constructor {
	...
	maskScale = _maskScale;
	maskSmoothness = _maskSmoothness;
	noiseTexture = _noiseTexture ?? sprite_get_texture(__ppf_sprNoiseSimplex, 0);
...
function FX_Dithering(_enabled, _mode=0, _intensity=1, _bitLevels=8, _contrast=1, _threshold=0, _scale=1, _bayerTexture=undefined, _bayerSize=8) : __PPF_FXSuperClass() constructor {
	...
	threshold = _threshold;
	scale = _scale;
	bayerTexture = _bayerTexture ?? sprite_get_texture(__ppf_sprBayer8x8, 0);
...
function FX_NoiseGrain(_enabled, _intensity=0.3, _luminosity=0.0, _scale=0.5, _speed=true, _mix=1, _noiseTexture=undefined, _noiseSize=256) : __PPF_FXSuperClass() constructor {
	...
	speedd = _speed;
	mix = _mix;
	noiseTexture = _noiseTexture ?? sprite_get_texture(__ppf_sprNoisePoint, 0);
