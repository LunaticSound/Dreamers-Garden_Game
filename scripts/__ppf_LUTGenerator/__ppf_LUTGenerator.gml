...
function PPFX_LUTGenerator() constructor {
	...
	static GenerateGrid = function(_squares=8, _exportAsSprite=true) {
		...
		Destroy();
		__lutType = "Grid";
		var _lutSize = power(_squares, 3); // cube
		
		var _surf = surface_create(_lutSize, _lutSize, __textureFormat);
		surface_set_target(_surf);
			draw_clear(c_black);
			shader_set(__ppf_shLUTMaker);
			shader_set_uniform_f(__uniLUTGridSize, _lutSize, _lutSize); // 512 x 512
			shader_set_uniform_f(__uniLUTGridSquares, _squares, _squares); // 8 x 8
			draw_sprite_stretched(__ppf_sprPixel, 0, 0, 0, _lutSize, _lutSize);
			shader_reset();
		surface_reset_target();
		...
		if (!__exportAsSprite) {
			...
			var _sprite = sprite_create_from_surface(_surf, 0, 0, _lutSize, _lutSize, false, false, 0, 0);
			surface_free(_surf);
			__output = _sprite;
		}
		return __output;
